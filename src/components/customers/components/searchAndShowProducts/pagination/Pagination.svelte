<script>
    ...
    const handleCount = (e) => {
        ...
        } else if (e.target.innerHTML == "Adelante" && desc && page < (customers.totalElements/5).toFixed(0)) {
            page += 1;
            debouncedSearch({'target':{'value': desc}}, page);
        }
    };
...
<div class="d-flex">
    ...
    <button disabled class="me-1 rounded">{(customers.totalElements/5).toFixed(0)}</button>
    {/if}
